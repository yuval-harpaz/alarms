...
rockets = readtable('~/Documents/alarm.csv');
...
XY = readtable('~/Documents/alarmXY.csv');
...
[~,order] = sort(XY.loc);
XY = XY(order,:);
writetable(XY,'~/Documents/alarmXY.csv','Delimiter',',','WriteVariableNames',true);
...
xlim([34 35.75])
for ii = 1:height(XY)
    rock(ii,1) = length(unique(rockets.time(ismember(rockets.loc,XY.loc{ii}))));
    plot(XY.X(ii),XY.Y(ii),'.k','MarkerSize',rock(ii)/3+1);
end
