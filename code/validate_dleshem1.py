...
ty = pd.to_datetime(dfy['time'])
skips = np.where(np.diff(dfl['rid'].values) > 1)[0]
print(dfl['rid'].values[skips+1] - dfl['rid'].values[skips])
summary = pd.DataFrame(columns=['from rid', 'to rid', 'gap', 'n missing','last rid time', 'first missing', 'last missing', 'next rid time'])
for skip in skips:
    t0 = pd.to_datetime(dfl['alertDate'][skip])
    t2 = pd.to_datetime(dfl['alertDate'][skip+1])
    candidates = np.where((ty - t0 > pd.Timedelta(seconds=30)) & (t2 - ty > pd.Timedelta(seconds=30)))[0]
    row = len(summary)
    summary.at[row, 'from rid'] = dfl['rid'][skip]
    summary.at[row, 'to rid'] = dfl['rid'][skip+1]
    summary.at[row, 'gap'] = dfl['rid'][skip+1] - dfl['rid'][skip]
    summary.at[row, 'n missing'] = len(candidates)

    if len(candidates) > 0:
        summary.at[row, 'first missing'] = ty.values[candidates[0]]
        summary.at[row, 'last missing'] = ty.values[candidates[-1]]
        summary.at[row, 'last rid time'] = pd.to_datetime(dfl['alertDate'][skip])
        summary.at[row, 'next rid time'] = pd.to_datetime(dfl['alertDate'][skip+1])
    else:
        summary.at[row, 'first missing'] = np.nan
        summary.at[row, 'last missing'] = np.nan
        summary.at[row, 'last rid time'] = np.nan
        summary.at[row, 'next rid time'] = np.nan
summary.to_csv('~/RedAlert/rid_skips_summary.csv', index=False)
    # if len(candidates) > 0:
    #     print(f'XXXXXXXX skip: {str(skip).zfill(6)} XXXXXXXXXXXXXXXX')
    #     print(f'before: {t0}')
    #     print(' '.join([str(t).split('.')[0] for t in ty.values[candidates]]))
    #     print(f'after: {t2}')
# llen = len(dfl)
