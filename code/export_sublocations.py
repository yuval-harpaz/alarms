...
sublocheb = {'אשקלון; בי"ח ברזילי': [31.6632444309682, 34.5578359719349],
             ...
             'פסטיבל נובה; מיגונית חניון רעים': [31.399979675305445, 34.47375717556573],
             ...
             'פסטיבל נובה; בר מרכזי':[31.398148261532118, 34.47002693739724],
             'פסטיבל נובה; בר': [31.398900, 34.470031],
             'פסטיבל נובה; פקק ביציאה':[31.400191046306386, 34.47355064596282],
...
##
subloc = np.unique(db['מקום האירוע'][(db['מקום האירוע'].str.contains(';') == True) & (db['event_coordinates'].isnull())].values)

for ii in range(len(subloc)):
    if subloc[ii] not in sublocheb.keys():
        print('missing: '+subloc[ii])
    else:
        rows = np.where((db['מקום האירוע'] == subloc[ii]) & (db['event_coordinates'].isnull()))[0]
        for row in rows:
            if np.isnan(db['event_coordinates'][row]):
                db.at[row, 'event_coordinates'] = str(sublocheb[subloc[ii]])[1:-1]
            else:
                raise Exception('not empty?')
db.to_excel('~/Documents/subloc.xlsx', index=False)
##
